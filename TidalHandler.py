...
def build_playlist(session, tidal_playlist, youtube_playlist, not_found):
    #Takes Youtube Playlist json and turns into a tidal playlist

    tidal_tracks = []
    for song in range(len(youtube_playlist["tracks"])):

        tracks = youtube_playlist["tracks"][song]
        ...
        except TypeError as e:  # If the Album info is not there
            find_track(session, t_name, artist, tidal_tracks)
...
def check_name(song_name, possible_match):
    ...
    if similarity_score > threshold: 
        print(f"Possible match between (Youtube) {song_name} and (Tidal) {possible_match}  {similarity_score}% please double check! Is this correct?: ")
        if input("y or n: ") == "y":
            return True
    else: return False
